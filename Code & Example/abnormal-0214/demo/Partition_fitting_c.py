...
def main(NUM):
    Jlabel = ['J27', 'J26', 'J25', 'J24']
    Tlabel = ['T27', 'T26', 'T25', 'T24']
    Rlabel = ['R27', 'R26', 'R25', 'R24']
    Clabel = ['C27', 'C26', 'C25', 'C24']
    Glabel = ['G2324', 'G2423', 'G2425', 'G2524', 'G2526', 'G2625', 'G2627', 'G2726']
    Olabel = ['O2324', 'O2423', 'O2425', 'O2524', 'O2526', 'O2625', 'O2627', 'O2726']
    ...
    wk_sheet.cell(row=40, column=6, value=H1)  # 在第12行，第6列下插入“H1”数值
    wk_sheet.cell(row=41, column=6, value=H2)
    wk_sheet.cell(row=42, column=6, value=H3)
    # wk_sheet.cell(row=43, column=6, value=H4)
    # wk_sheet.cell(row=44, column=6, value=H5)

    wk_sheet.cell(row=45, column=6, value=L1)  # 在第17行，第6列下插入“L1”数值
    wk_sheet.cell(row=46, column=6, value=L2)
    wk_sheet.cell(row=47, column=6, value=L3)
    wk_sheet.cell(row=48, column=6, value=L4)
    wk_sheet.cell(row=49, column=6, value=L5)

    # n2＞n1＞n3
    if n2 > n1 > n3:
        wk_sheet.cell(row=50, column=6, value=n0)
        wk_sheet.cell(row=51, column=6, value=n1)
        wk_sheet.cell(row=52, column=6, value=n2)
        wk_sheet.cell(row=53, column=6, value=n3)
    else:
        wk_sheet.cell(row=50, column=6, value=n0).fill = fille
        wk_sheet.cell(row=51, column=6, value=n1).fill = fille
        wk_sheet.cell(row=52, column=6, value=n2).fill = fille
        wk_sheet.cell(row=53, column=6, value=n3).fill = fille


    wk_sheet_1 = wb['拟合程度']
    wk_sheet_1.cell(row=4, column=29, value=correlation_coefficient1)
    wk_sheet_1.cell(row=4, column=30, value=correlation_coefficient2)
    wk_sheet_1.cell(row=4, column=31, value=correlation_coefficient3)
    wk_sheet_1.cell(row=4, column=32, value=correlation_coefficient4)
    wk_sheet_1.cell(row=4, column=33, value=correlation_coefficient5)
    wk_sheet_1.cell(row=4, column=34, value=correlation_coefficient6)
    ...
    #
    # plt.title(NUM)
    # plt.savefig(NUM + '/c.png', dpi=400)
    # plt.show()


# all = ['1-曾悦', '10-宋群英', '11-宋文', '12-唐嘉仪',  '14-王玮璇', '15-谢川霞', '16-谢容', '17-徐玲玲',
